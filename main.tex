...
\begin{theorem}[Optimailty via Policy Improvement]
...
Before, our (random) approximation of \( V^\pi(s) \) after \( m \in \left\{  1,\mydots,M \right\}\) roll-outs takes the form 
\[
...
This update rule is much more memory efficient, as we do not have to store the rewards of the previous rollouts \( l = 1,..,m \).
...
\begin{itemize}
    ...
    \item The advantage is, that it requires less samples, compared to the original approach, in which each roll-out is only used once.
...
\[
    \hat{V}_{m+1}^{\pi} (s) = \hat{V}_m^\pi(s) + \alpha_{m+1}(s) \cdot \left[ \sum_{n=0}^N \gamma^n \cdot r_n^m - \hat{V}_m^\pi(s) \right]
\]
...
\[
    \hat{V}_{m+1}^\pi(s) = \hat{V}_m^\pi(s) + \alpha_{m+1}(s) \cdot \left[ r + \gamma \cdot \hat{V}_m^\pi(s') - \hat{V}_m^\pi(s) \right]
\]  
