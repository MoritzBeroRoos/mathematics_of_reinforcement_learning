...
\]
For any \( \pi \in \Pi  \), we then write \( PP_s^\pi := PP_{\delta_s}^\pi\) for the expectation operator with respect to \( PP_\mu^\pi \) and let \( \mathbb{E}_s^\pi = \mathbb{E}_{\delta_s}^\pi \).
...
\[
    \sum_{n=0}^{\infty} \gamma^n | r(S_n, A_n) \leq C \mathbb{P}_s^\pi-\text{a.s.}
\] 
...
\begin{theorem}[Bellman Equation]
    Let \( s \in S, \pi   \). Then 
    \begin{align*}
                V^\pi(s) &= \sum_{s'  S, a \in A} \left[ r(s,a) + \gamma V^\pi(s')  \right] (a | s) p(s' | s,a)\\
